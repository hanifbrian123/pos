<div id="{{ $id }}" class="modal-backdrop fixed inset-0 bg-[#0F172A]/40 backdrop-blur-sm z-[60] flex items-center justify-center hidden">
    <div class="bg-white w-full max-w-sm rounded-2xl shadow-xl p-8 flex flex-col items-center text-center animate-in fade-in zoom-in duration-200">
        ...
        <div class="grid grid-cols-2 gap-3 w-full">
            <button data-close-modal='{{ $id }}' class="py-3 border border-[#E2E8F0] rounded-xl font-semibold text-[#64748B] hover:bg-[#F8FAFC] transition-colors">
                {{ $noDesc }}
            </button>
            <button  class="py-3 bg-red-500 text-white rounded-xl font-semibold hover:bg-red-600 transition-colors">
                {{ $yesDesc }}
            </button>
        </div>
    </div>
